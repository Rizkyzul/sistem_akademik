...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (!$attemptSuccess) {
            ...
            // Periksa jika mahasiswa ada DAN punya relasi ke user
            if ($mahasiswa && $mahasiswa->user) {
                
                // Ambil data user yang terkait
                $userToLogin = $mahasiswa->user;

                // VERIFIKASI PASSWORD SECARA MANUAL
                // Cek apakah password yang dikirim ($password) 
                // cocok dengan hash password di database ($userToLogin->password)
                if (Hash::check($password, $userToLogin->password)) {
                    
                    // Password benar! Login-kan user ini secara manual.
