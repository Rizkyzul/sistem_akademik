   ...
   public function lihatDaftarMahasiswa(Request $request)
{
    ...
    // --- 2. Ambil semua kelas yang diajar oleh dosen ---
    ...
    // --- 3. Ambil semua mahasiswa berdasarkan kelas yang diajar ---
    // Jika tidak ada kelas diajar (belum terdaftar di pengampu), tampilkan semua mahasiswa (fallback)
    $mahasiswas = $kelasDiajarOlehDosen->isNotEmpty()
        ? Mahasiswa::whereIn('kelas', $kelasDiajarOlehDosen)->orderBy('nama')->get()
        : Mahasiswa::orderBy('nama')->get();

    // --- 4. Ambil semua mata kuliah yang diajar oleh dosen untuk filter dropdown ---
    $mataKuliahsDiajar = PengampuMataKuliah::where('dosen_id', $dosen->id)
        ->with('mataKuliah')
        ->get()
        ->unique('mata_kuliah_id')
        ->sortBy(fn ($item) => optional($item->mataKuliah)->nama_mk);

    // --- 5. Ambil status presensi hari ini (mahasiswa yang hadir) ---
    ...
    if ($pengampuIdsHariIni->isNotEmpty() && $mahasiswas->isNotEmpty()) {
        $presensiStatusHariIni = PresensiMahasiswa::whereIn('mahasiswa_id', $mahasiswas->pluck('id'))
            ...
            ->where('status_kehadiran', 'Hadir')
            ->pluck('mahasiswa_id')
            ->flip(); // flip untuk pengecekan cepat via isset()
    }

    // --- 6. Debugging opsional (aktifkan bila data kosong) ---
    /*
    dd([
        'dosen' => $dosen->nama ?? null,
        'kelas_diajar' => $kelasDiajarOlehDosen,
        'mahasiswa_kelas' => Mahasiswa::pluck('kelas')->unique(),
        'jumlah_mahasiswa' => $mahasiswas->count(),
        'hari_ini' => $hariIni,
        'pengampu_ids' => $pengampuIdsHariIni,
        'presensi_hadir' => $presensiStatusHariIni->keys(),
    ]);
    */
